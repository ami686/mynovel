"""
new_post_video.py —— 新建视频文章（只负责算 slug 和 date，不决定存哪）
（本脚本编号只基于 content/video/ 目录，跟其他分类的编号池互不相关）

逻辑：
  1. 只扫描 content/video/ 下所有 .md 文件（跳过 _index.md），
     找出已有编号里最大的数字，算出下一个编号
  2. 打印提示：扫描到几篇已有编号文章、新文章编号是多少
  3. 自动打开记事本，弹出一个临时文件，里面已经填好 slug 和当前时间
  4. 你在记事本里写标题和正文，写完自己"另存为"到 content/video/ 文件夹，
     文件名随便起，不影响 slug（slug 是写在内容里的，不是看文件名）

用法：双击运行，或 python new_post_video.py
"""
...
# 只扫描这个顶级目录（相对于 CONTENT_DIR），独立编号池
TARGET_TOP_DIR = "video"
# ────────────────────────────────────────────────────────
...
def scan_max_slug(content_dir):
    ...
    target_dir = os.path.join(content_dir, TARGET_TOP_DIR)
...
def main():
    scan_dir = os.path.join(CONTENT_DIR, TARGET_TOP_DIR)
    ...
    print(f"已扫描到 {total_with_slug} 篇已有编号视频文章")
    print(f"新视频文章编号为: {next_slug}\n")
    ...
    tmp_path = tmp_dir / f"新视频文章_{next_slug}.md"
    tmp_path.write_text(content, encoding="utf-8")

    print("即将打开记事本，请填写标题并写正文。")
    print("写完后请「另存为」到 content/video/ 文件夹，文件名随意起。\n")
    input("按回车打开记事本...")
