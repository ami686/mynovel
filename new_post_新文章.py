...
（本脚本只给「非 video」文章算编号，content/video/ 不参与，video 请用另一个脚本单独处理）

逻辑：
  1. 递归扫描 content/ 下所有 .md 文件（跳过 _index.md、跳过 content/video/），
...
# 需要排除的顶级目录（不参与本脚本的扫描和编号），目录名相对于 CONTENT_DIR
EXCLUDE_TOP_DIRS = {"video"}
# ────────────────────────────────────────────────────────
...
def scan_max_slug(content_dir):
    ...
    for root, dirs, files in os.walk(content_dir):
        # 如果当前是 content_dir 的顶层，从 dirs 里剔除要排除的目录，
        # 这样 os.walk 后续不会再往这些目录里递归
        if os.path.abspath(root) == os.path.abspath(content_dir):
